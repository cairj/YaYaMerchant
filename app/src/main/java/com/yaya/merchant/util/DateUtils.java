...
public class DateUtils extends android.text.format.DateUtils {
    ...
    //将格式化以后的时间转成日期
    public static long getTime(String generateTime){
        long time=0l;
        String[] split=generateTime.split(":");
        long[] splitTime=new long[split.length];
        for (int i=0;i<split.length;i++){
            splitTime[i]=Long.parseLong(split[i]);
        }

        if (splitTime.length==3){
            time=splitTime[0]*3600+splitTime[1]*60+splitTime[2];
        }else if (splitTime.length==2){
            time=splitTime[0]*60+splitTime[1];
        }
